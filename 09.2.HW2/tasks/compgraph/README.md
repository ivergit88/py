...
1. Создайте git-ветку `compgraph` для решения задачи. Рекомендации:
   ...
   * Если делаете это впервые, посмотрите обязательно лекцию `8.2.GitBranches` и почитайте секцию `Как оформить merge request`
   * Создайте ветку ДО ТОГО, как начали делать задание
   * Не забывайте переключаться обратно на `main`, когда/если будете коммитить другие задачи

2. Реализуйте ваш вариант библиотеки для создания и использования вычислительных графов. Подумайте над существующим интерфейсом. Его можно менять, но тесты должны работать.
...
3. В папке `tests`(*подпапки `tests/correctness` и `tests/memory` модифицировать **нельзя***) напишите юнит-тесты на
   * все MapReduce-операции, на которые нет тестов в либе (актуально, если вы будете добавлять свои операции)
   * все методы интерфейса графа
   * проверку того, что что последовательные запуски одного и того же графа не влияют друг на друга
   
   **Важно:** Code coverage должен быть больше `95%`. 
   ...
   P.S. Если вам для достижения лимита 95% coverage не пришлось (или почти не пришлось) писать дополнительные тесты - это нормально. Но помните, что 100% покрытия не всегда означает хорошие тесты. Думайте.
...
5. Нужно добиться того, чтобы каждую задачу можно было как-то запустить. Если вы не идёте на ревью, то самый простой способ - на каждую задачу добавить отдельный скрипт, позволяющий быстро запустить задачу на входных данных из папки `resources`.
   ...
   А если вы идёте на ревью и/или очень любите CLI, то тогда сделайте entry points для вашей библиотеки так же, как вы это делали, например, в задачке `steganography_tool` (используйте `click`/`cyclopts`/`typer`/etc.)
...
6. Напишите Readme: как установить, пользоваться библиотекой, запустить примеры и тесты. (Вариант темплейта [Readme](https://github.com/PurpleBooth/a-good-readme-template))  
   Сам ридми нужно назвать `LIB_README.md` (а не `README.md`, чтобы не было коллизий с этим файлом). LLM-ки в помощь. *бро, you're absolutely right и т. д.*

7. Подкорректируйте `pyproject.toml` своей метаинформацией (особенно актуально, если вы собираетесь делать там entrypoint)

8. Отправьте Merge Request, поставьте ему тег (label) `compgraph` и проверьте корректность MR по чек-листу

9. Ждите проверки `MR`, баллы выставляются вручную после дедлайна задачи.  **Важно:** НЕ вмердживайте чертов MR сами. Ждите проверки!!!

**Важно!** Нет тега -> мы вас не найдём и не проверим -> 0 баллов
...
Т.е., у вас есть возможность его сделать, поглядеть, что всё ок, исправить и после этого проставить тег.


#### Ревью

* +100 баллов бонуса, если поправите ВСЕ замечания ревьювера (или аргументируете, почему какое-то замечание править не нужно)
...
**Важно!** Чтоб участвовать в ревью - поставьте тег `review` на ваш merge request.

У нас мало ревьюверов, так что если желающих на ревью будет много, то возьмем только N (~10-20) первых реквестов по времени последнего апдейта. Кто **раньше** добавил последнее исправление - проходит.
Число реквестов, которые мы можем отсмотреть, ещё пока уточняется; есть шансы, что посмотрим всех.

### Про процесс ревью

На ревью вашу работу с точки зрения стиля/дизайна/"best practice" будет проверять ревьювер из нашей команды.  
У вас будет 1-2-3 итерации, где вам будут присылать замечания на исправления. Замечаний может быть очень много. Замечания n-ной итерации могут быть даже про код, который был написан ещё на первой (тогда пропустил, сейчас заметил). 

Проверять начнем после наступления дедлайна. Исправлять можно до конца учебного года. Скорее всего, исправление будет идти вместе с HW3, так что подумайте, надо ли вам это :)

Во время ревью мы ставим теги:
* `fix it` - поправить код по комментариям
* `reviewed` - ревью завершено, оценка выставлена

Когда почините код, ставьте теги
* `questions` - если есть вопросы
* `fixed` - если можно смотреть правки. Снимайте наш тег `fix it` в этом случае
...
* ✅ Стоит тег `compgraph` (иначе мы даже не увидим ваш MR)
...
* ✅ Есть скрипты/CLI на каждую задачу
...
- проставляем лейбл `compgraph` (и `review`, если хотите его проходить) и нажимаем submit merge request (подробности на скрине) 
   <details><summary><a>Картинка от Никиты, актуально каждый год</a></summary><img src="https://avatars.mds.yandex.net/get-pdb/2031002/0148eedb-dbb9-45cf-bf31-cef79456f5cc/s1200" width=800/></details>

### Про задачу

В этом задании вы продолжите работать над библиотекой для вычислений над графами.

Напомним определение таблицы из `diesel_power`:

```
Таблица - это последовательность некоторых словарей,
...
Вот так может выглядеть (спойлер: он так и выглядит, см. [`compgraph/algorithms.py`](compgraph/algorithms.py)) граф,
...
После описания граф нужно уметь запускать, передав ему все нужные входные данные, 
условно каким-нибудь методом `run`:
```python
...
Отсортировать произвольно большие данные в потоковой манере невозможно, и это алгоритмически сложный примитив; поэтому в рамках нашего задания
...
(shad_env)$ uv pip install -e compgraph --force-reinstall
...
вхождение (нас интересует количество документов, в которых встретилось слово; без разницы, сколько раз).
...
P. S. В датасете есть пара строчек, где enter_time позже, чем leave_time. 
