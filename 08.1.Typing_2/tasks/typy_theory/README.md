...
В файле `Problems.pdf` находится 9 задач. В файлике `theory.py` нужно написать, на каких строчках mypy будет ругаться и почему.
...
Чтобы запускать typecheck в jupyter-ноутбуке, посмотрите SETUP.md
