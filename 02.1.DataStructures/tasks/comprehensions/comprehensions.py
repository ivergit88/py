...
def get_unique_page_ids(records: list[tp.Mapping[str, tp.Any]]) -> set[int]:
    ...
    """
    return {r["PageID"] for r in records if r.get("PageID") is not None}
...
def get_unique_page_ids_visited_after_ts(records: list[tp.Mapping[str, tp.Any]], ts: int) -> set[int]:
    ...
    """
    return {r["PageID"] for r in records if r.get("PageID") is not None and r.get("EventTime", 0) > ts}
...
def get_unique_user_ids_visited_page_after_ts(
    ...
    return {
        r["UserID"]
        for r in records
        if r.get("UserID") is not None and r.get("EventTime", 0) > ts and r.get("PageID") == page_id
    }
...
def get_events_by_device_type(
    ...
    """
    return [r for r in records if r.get("DeviceType") == device_type]
...
def get_region_ids_with_none_replaces_by_default(
    ...
    """
    return [
        r.get("RegionID", DEFAULT_REGION_ID) if r.get("RegionID") is not None else DEFAULT_REGION_ID
        for r in records
    ]
...
def get_region_id_if_not_none(
    ...
    :return: region ids
    """
    return [r["RegionID"] for r in records if r.get("RegionID") is not None]
